...
// With information provided from The Random User Generator API, send a request to the API
  // and use the dad to display 12 users, along with some basic information for each

  $(document).ready(function() {
    let api = 'https://randomuser.me/api/?results=12&nat=us,ca&exc=gender,login,registered,id,phone';
    $.getJSON(api, function(data){
      let employeeData = data.results; // Stores the array in a variable to use for the modal window
      console.log(employeeData, 'employeeData variable'); // Console.log test to make sure the variable is stored correctly
  
  // Get and display 12 random users
      let employeeHTML = ''; 
        $.each(data.results, function(index, employee){
          // The following data will display 12 users and some additional information
  ...
  //Modal window
  let modalWindow;
  $('body').append('<div id="modal-window-div">'); // Creates a div to put the modal window in
  
      // This section finds the index of each employee in the gallery
      $('.card').click(function(employeeData){
        ...
        function createModal() {
          ...
          $('#modal-window-div').html(modalWindow); 
